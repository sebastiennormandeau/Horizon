...
- **Credit cards & joint accounts**: a purchase is counted at the moment of purchase, so a credit-card charge depletes a pot immediately; the later *payment* of that card from a chequing account is a transfer between the household's own accounts and must not count again. Sync auto-assigns such rows to the `Transfer` bucket, detected on Plaid's **detailed** category (`LOAN_PAYMENTS_CREDIT_CARD_PAYMENT`, `TRANSFER_*_ACCOUNT_TRANSFER`) — the *primary* category would be too broad, `LOAN_PAYMENTS` also covers mortgages. `Transfer` and `Archived` are deliberately absent from `VALID_BUCKETS` so the ledger ignores them; `Transfer` is also excluded from reports, `Archived` is not (those are real expenses, just not worth sorting). `Archived` holds pre-current-month rows, auto-applied on a connection's **first** sync only. A connection flagged `is_joint` copies `is_joint_account` onto its transactions, and the ledger then skips the internal-debt adjustment — without it, a joint account linked under one member makes the debt grow forever, since Plaid attributes every transaction to whoever established the connection.
